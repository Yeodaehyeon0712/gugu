...
public class IntFormulaCalculator : FormulaCalculator
{
    int _lastComputeVariable;
    int _prevRefFuncZeroPoint = 0;
    public IntFormulaCalculator(string formula) : base(formula)
    {
        Formula = formula;
    }
    void Recompute(long level)
    {
        ...
        _lastComputeVariable = Convert.ToInt32(_dataTable.Compute(val, string.Empty));
    }
    public int GetValue(long level)
    {
        if (_lastLevel != level)
            Recompute(level);
    ...
    protected override string ComputeFunctionRef(string val)
    {
        ...
        if (split.Length == 2) _prevRefFuncZeroPoint = int.Parse(split[1]);
